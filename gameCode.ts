...
let player_loc:number[]; 
let scene:string; //For telling the code whether the player is at the title screen, in game, at the end of the game, or anything else
let room_index:number;
let room:string[][] =[[""]];
let sounds:HTMLAudioElement[] = [new Audio("door.wav"), new Audio("locked.wav"), new Audio("click.wav"), new Audio("metal.wav")];

let info:string[] = ["","","","","",""]; //Is the array for 
function addToCurrentInfo(newInfo:string) { info[0] += " | " + newInfo; }
function addInfo(newInfo:string) { info = info.slice(0,5); info.unshift(newInfo); }

let room_sizes:number[][] = [[19,6],[5,3]]; //,[],[],[],]
let room_objects:room_object[][] = [[ {chr:'@', col:2, row:3, color:"red", sound_index:2, active:false, connects_to_door:1}, {chr:'/', col:8, row:5, active:false, door_goes_to_room:1, door_spawn_loc:[1,2], sound_index:3}, {chr:'#', col:14, row:5, text_box:"The sign reads 'Welcome to the shifting walls facility. Here, we reserach the future of bigger-on-the-inside technology'"} ],
...
interface room_object { //Interfaces work very similarly to structs but without the methods
    chr:string;   //The character that is used to display the object
    col:number;   //X coordinate
    row:number;   //Y coordinate
    color?:string; //The color of the object
    sound_index?:number;

    interaction_index?:number; //Used in the interaction function to find out what happens.
    text_box?:string;          //If it can be interacted with to show text, this is where the text box lives
    active?:boolean;           //Used for both switches and for doors 
    connects_to_door?:number   //Contains the index of the door object that the door might connect to

    ends_the_game?:boolean;      //If going through the door would end the game
    door_perma_locked?:boolean; //If the player cannot open the door behind them 
    door_goes_to_room?:number;  //The index of the room the door takes you to
    door_spawn_loc?:number[];   //The place that the player spawns in the room they're taken to
    heavy_door?:boolean;        //If the door it goes to is permalocked
}

function create_room(player_spawn:number[]) { //This is how TypeScript handles function definitions
    //Takes string array input, outputs 2D string array. 
    let room_size:number[] = room_sizes[room_index];
    let objects:room_object[] = room_objects[room_index];
    let output:string[][] = [];
    ...
    for (let thing of objects) output[thing.row][thing.col] = thing.chr; //Place the room's objects 

    room = output;
    //return [ ["place","holder"],["out","put"] ]
}

function display_room():void {
    //Deep copy the room array so that editing doesn't mess with the room itself
    let using_room:string[][] = [];
    for (let i=0; i<room.length; i++) {
        let inner_use:string[] = [];
        for (let j=0; j<room[i].length; j++) inner_use.push(room[i][j]);
        using_room.push(inner_use);
    }

    //Apply color step
    for (let thing of room_objects[room_index]) { 
        if (thing.color !== undefined) { using_room[thing.row][thing.col] = `<span style="color: ${thing.color}">${thing.chr}</span>`}
    }
...
function move_character(direction:string):void {
    let target:number[] = [-1,-1];
    switch(direction) {
        case "up":    target = [player_loc[0] - 1, player_loc[1]]; break; //Decrease row by 1
        case "left":  target = [player_loc[0], player_loc[1] - 1]; break; //Decrease col by 1
        case "down":  target = [player_loc[0] + 1, player_loc[1]]; break; //Increase row by 1
        case "right": target = [player_loc[0], player_loc[1] + 1]; break; //Increase col by 1
    } if (target[0] == -1 && target[1] == -1) return; //If the invalid direction error, exit function

    let going_to:string = room[target[0]][target[1]];
    if (going_to == '.') { //If empty space
        ...
        */
        ...
        /*1*/ let door:room_object = {chr:'invalid', col:0, row:0, color:''};
        /*1*/ for (let thing of room_objects[room_index]) { if (thing.row == target[0] && thing.col == target[1]) door = thing; }

        if (door.door_perma_locked) addInfo("Don't go back. Keep searching for them."); 
        else if (door.ends_the_game !== undefined) end_game();
        else if (!door.active) { addInfo("Locked door."); sounds[1].play(); }
        else if (door.active) {
            /*2*/ room_index = door.door_goes_to_room as number; //Has to include 'as number' because the property is 'number | undefined'
            /*2*/ let spawn_loc:number[] = door.door_spawn_loc as number[];
...
function find_interaction() {
    ...
    let target:number[];
    let validInteracts:string[] = ['@','#','/'];
    for (let i=0; i<4; i++) { 
        let tempCheck:string = room[targets[i][0]][targets[i][1]];
    ...
    if (target === undefined) return //If target is undefined, that means that the player is not interacting with anything

    let object:room_object = {chr:'invalid key', col:0, row:0, color:''}; //For 
    ...
    if (object.chr == '/') {
        if (object.door_perma_locked) addInfo("Don't go back. Keep searching for them."); 
        else if (object.ends_the_game !== undefined) end_game();
        else if (!object.active) { addInfo("Locked door."); sounds[1].play(); }
        else if (object.active) {
            /*2*/ room_index = object.door_goes_to_room as number; //Has to include 'as number' because the property is 'number | undefined'
            /*2*/ let spawn_loc:number[] = object.door_spawn_loc as number[];
            /*3*/ create_room(spawn_loc);
            /*3*/ player_loc = spawn_loc;
        ...
        if (object.connects_to_door !== undefined) { //If the button connects to a door
            ...
            let to_door:number = object.connects_to_door;
        ...
        } else interaction(object.interaction_index); //If it's not a normal door button, then run the interaction code.
    }
}

function interaction(index:number) {

}

function process_input(key:string):void { //Will only accept vaild keys
    ...
    if (invalidKey || scene == 'end') {} else { display_room(); }
}

function title_screen():void {
    scene = "title";
    let title:string[] = [ "<pre>", //The <pre> tag is there to retain all of the spaces in the title screen
        "███████╗ █████╗  ██████╗██╗██╗     ██╗████████╗██╗   ██╗    ██╗  ██╗██╗   ██╗███╗   ██╗████████╗",
        "██╔════╝██╔══██╗██╔════╝██║██║     ██║╚══██╔══╝╚██╗ ██╔╝    ██║  ██║██║   ██║████╗  ██║╚══██╔══╝",
        "█████╗  ███████║██║     ██║██║     ██║   ██║    ╚████╔╝     ███████║██║   ██║██╔██╗ ██║   ██║",
        "██╔══╝  ██╔══██║██║     ██║██║     ██║   ██║     ╚██╔╝      ██╔══██║██║   ██║██║╚██╗██║   ██║",   
...
function start_game():void { //Set all of the global variables for the start of the game
    player_loc = [2,9];
    scene = "game";
    room_index = 0;
...
window.addEventListener('keydown', function (e: KeyboardEvent):void { 
    if (scene == "end") return; //Nothing can be done at the end of the game
    else if (scene == "title") {
        if (e.key == "Enter") start_game();
    } else process_input(e.key);
  }, false);